...
class RegistrarQuery
{
    ...
    public function __construct()
    {
        ...
        $this->Queries["SELECTResPer"] = "SELECT PER.Nombres,PER.ApellidoPaterno,PER.ApellidoMaterno FROM reservacionespersonal AS RSPE INNER JOIN personal AS PER ON PER.IDPersonal=RSPE.IDPersonalRes WHERE RSPE.IDReserva=:idr AND PER.IDPersonal=:id AND RSPE.FechaReserva=:fch";

        $this->Queries["SELECTResPro"] = "SELECT ACAD.Nombre,ACAD.ApellidoParternoProfesor,ACAD.ApellidoMarternoProfesor FROM reservacionespersonal AS RSPE INNER JOIN academicos AS ACAD ON ACAD.IDProfesor=RSPE.IDPersonalRes WHERE ACAD.IDProfesor=:id AND RSPE.FechaReserva=:fch AND RSPE.IDReserva=:idr";
        ...
        $this->Queries["INSERTAsisPer"] = "INSERT INTO asistenciaspersonal (FechaAsistencia,HoraAsistencia,IDPersonal) SELECT :fch,:hr,:id FROM DUAL WHERE NOT EXISTS(SELECT IDPersonal,FechaAsistencia FROM asistenciaspersonal WHERE IDPersonal=:id AND FechaAsistencia=:fch)";

        $this->Queries["INSERTAsisPro"] = "INSERT INTO asistenciasacademicos (FechaAsistencia,HoraAsistencia,IDPersonal) SELECT :fch,:hr,:id FROM DUAL WHERE NOT EXISTS(SELECT IDPersonal,FechaAsistencia FROM asistenciasacademicos WHERE IDPersonal=:id AND FechaAsistencia=:fch)";
    }
