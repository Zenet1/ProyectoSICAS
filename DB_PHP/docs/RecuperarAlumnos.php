...
function RecuperarAlumnos(PDO $Conexion)
{
    ...
    $insertar = "INSERT INTO alumnos (Matricula, NombreAlumno, ApellidoPaternoAlumno, ApellidoMaternoAlumno, IDPlanEstudio, CorreoAlumno, Genero, IDUsuario, IDModelo, NivelEducativo) SELECT ?,?,?,?,?,?,?,?,?,? FROM DUAL WHERE NOT EXISTS(SELECT Matricula FROM alumnos WHERE Matricula=?) LIMIT 1";
    ...
    foreach ($archivo as $linea) {
        ...
        $insertar_obj->execute(array($datos_archivo[0], $datos_archivo[1], $datos_archivo[2], $datos_archivo[3], $IDPlan["IDPlanEstudio"], $datos_archivo[8], $datos_archivo[4], $IDUsuario["IDUsuario"], 1, "Licenciatura", $datos_archivo[0]));
    }
}
