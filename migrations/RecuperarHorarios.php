<?php
    ...
    $sqlInsert = "INSERT INTO sicas_test.horarios (IDGrupo, IDPlanEstudio, Dia, HoraInicioHorario, HoraFinHorario, IDSalon) VALUES (?, ?, ?, ?, ?, ?);";
    $sqlrecuperarIDGrupo = "SELECT IDGrupo FROM grupos WHERE ClaveGrupo=?";
    $sqlrecuperarIDPlan = "SELECT IDPlanEstudio FROM planesdeestudio WHERE ClavePlan=?";
    ...
    $obj_recuperarIDPlan = $DB_CONEXION->prepare($sqlrecuperarIDPlan);
    ...
    foreach($archivo as $linea){
        ...
        $obj_recuperarIDPlan->execute(array($data[0]));
        ...
        if(isset($IDEdificio["IDEdificio"])){
            ...
            $IDPlan = $obj_recuperarIDPlan->fetch(PDO::FETCH_ASSOC);
            $IDSalon = $obj_recuperarSalon->fetch(PDO::FETCH_ASSOC);
            if(isset($IDSalon["IDSalon"])){
                $obj_insert->execute(array($IDGrupo["IDGrupo"], $IDPlan["IDPlanEstudio"], $data[6], $data[7], $data[8], $IDSalon["IDSalon"]));
            }
        }else{
            continue;
        }
